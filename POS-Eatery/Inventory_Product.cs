...
namespace POS_Eatery
{
    public partial class Inventory_Product : Form
    {
        ...
        private void View_Product_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                MySqlConnection cnn = new MySqlConnection(ConfigurationManager.ConnectionStrings["cnstring"].ConnectionString);
                cnn.Open();
                MySqlCommand cmdn = new MySqlCommand("SELECT * from table_config_percentage where Branch='" + branch.Text + "'", cnn);
                MySqlDataReader drn = cmdn.ExecuteReader();
                try
                {
                    if (drn.Read())
                    {
                        config_percent.Text = (string)drn.GetValue(1).ToString();
                       // default_amount.Text = (string)drn.GetValue(2).ToString();
                      //  default_amount.Text = (Convert.ToDecimal(default_amount.Text) / Convert.ToDecimal(100)).ToString();
                    }
                    else
                    {
                        config_percent.Text = "0";
            ...
            // gclass.Expand_Database("update table_stock_inventory_summary set quantity_out_litre=Round(Quantity_Out*1.75,2),quantity_in_litre=Round(Quantity_in*1.75,2),quantity_left_litre=Round(Quantity_left*1.75,2),quantity_out_bottle=Round(Quantity_Out/12.5,2),quantity_in_bottle=Round(Quantity_in/12.5,2),quantity_left_bottle=Round(Quantity_left/12.5,2),quantity_out_percent=Round(Quantity_Out*1.75/681.37412,3),quantity_in_percent=Round(Quantity_in*1.75/681.37412,3),quantity_left_percent=Round(Quantity_left*1.75/681.37412,3)");
            gclass.Expand_Database("update table_stock_inventory_summary set quantity_out_litre=Round(Quantity_Out*1.75,2),quantity_in_litre=Round(Quantity_in*1.75,2),quantity_left_litre=Round(Quantity_left*1.75,2),quantity_out_bottle=Round(Quantity_Out/12.5,2),quantity_in_bottle=Round(Quantity_in/12.5,2),quantity_left_bottle=Round(Quantity_left/12.5,2),quantity_out_percent=Round(Quantity_Out*1.75/" + Convert.ToDecimal(config_percent.Text) + ",3),quantity_in_percent=Round(Quantity_in*1.75/" + Convert.ToDecimal(config_percent.Text) + ",3),quantity_left_percent=Round(Quantity_left*1.75/" + Convert.ToDecimal(config_percent.Text) + ",3)");
